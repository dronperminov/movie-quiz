...
class KinopoiskParser:
    def __init__(self, tokens: List[str], logger: logging.Logger) -> None:
        ...
        self.image_types = ["frame", "screenshot", "shooting", "still"]
    ...
    def parse_movies(self, movie_ids: List[int]) -> List[dict]:
        ...
        return [self.__parse_movie(movie) for movie in movies]
    ...
    def __get_movies(self, query_params: List[str]) -> List[dict]:
        ...
        return self.__list_request(f'/v1.4/movie?{"&".join(params)}')
    ...
    def __get_images(self, query_params: List[str]) -> List[dict]:
        ...
        return self.__list_request(f'/v1.4/image?{"&".join(params)}')

    def __list_request(self, url: str) -> List[dict]:
        response = self.__api_request(url)
        docs = response["docs"]
        self.logger.info(f'docs: {response["pages"]} pages')

        while response["page"] < response["pages"]:
            self.logger.info(f'docs {response["page"] + 1} / {response["pages"]}')
